...
struct Pixel {
  ...
  std::string AnsiBg() const { return "\033[48;" + AnsiColor() + "m"; }
  std::string AnsiFg() const { return "\033[38;" + AnsiColor() + "m"; }
};
  ...
  void Render() override {
    ...
    for (int char_y = 0; char_y < height; ++char_y) {
      ...
      for (int char_x = 0; char_x < width; ++char_x) {
        ...
        std::string new_bg =
            result.bg.a < 0.5 ? ansi::kResetBG : result.bg.pixel().AnsiBg();
        if (new_bg != last_bg) {
          result_raw += new_bg;
          last_bg = new_bg;
        }
