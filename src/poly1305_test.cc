#include "poly1305.hh"

#include "gtest.hh"

#include "arr.hh"
#include "hex.hh"

using namespace maf;

char kNaClKey[32] = {0xee, 0xa6, 0xa7, 0x25, 0x1c, 0x1e, 0x72, 0x91,
                     0x6d, 0x11, 0xc2, 0xcb, 0x21, 0x4d, 0x3c, 0x25,
                     0x25, 0x39, 0x12, 0x1d, 0x8e, 0x23, 0x4e, 0x65,
                     0x2d, 0x65, 0x1f, 0xa4, 0xc8, 0xcf, 0xf8, 0x80};

char kNaClMsg[] = {
    };

TEST(Poly1305Test, NaCl) {
  Poly1305 poly1305(kNaClMsg, kNaClKey);
  EXPECT_EQ(BytesToHex(poly1305.bytes), "f3ffc7703f9400e52a7dfb4b3d3305d9");
}

TEST(Poly1305Test, NaClChunks) {
  Poly1305::Builder ctx(kNaClKey);
  ctx.Update(Span<>(kNaClMsg + 0, 32));
  ctx.Update(Span<>(kNaClMsg + 32, 64));
  ctx.Update(Span<>(kNaClMsg + 96, 16));
  ctx.Update(Span<>(kNaClMsg + 112, 8));
  ctx.Update(Span<>(kNaClMsg + 120, 4));
  ctx.Update(Span<>(kNaClMsg + 124, 2));
  ctx.Update(Span<>(kNaClMsg + 126, 1));
  ctx.Update(Span<>(kNaClMsg + 127, 1));
  ctx.Update(Span<>(kNaClMsg + 128, 1));
  ctx.Update(Span<>(kNaClMsg + 129, 1));
  ctx.Update(Span<>(kNaClMsg + 130, 1));
  auto mac = ctx.Finalize();
  EXPECT_EQ(BytesToHex(mac.bytes), "f3ffc7703f9400e52a7dfb4b3d3305d9");
}

TEST(Poly1305Test, Wrap) {
  char key[32] = {
      0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  };
  char msg[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

  Poly1305 poly1305(msg, key);
  EXPECT_EQ(BytesToHex(poly1305.bytes), "03000000000000000000000000000000");
}

TEST(Poly1305Test, Multiple) {
  char total_key[32] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                        0xff, 0xfe, 0xfd, 0xfc, 0xfb, 0xfa, 0xf9,
                        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

  Poly1305::Builder total_ctx(total_key);

  for (int i = 0; i < 256; i++) {
    /* set key and message to 'i,i,i..' */
    char key_i[32];
    for (int j = 0; j < sizeof(key_i); j++)
      key_i[j] = i;
    char msg_i[i];
    for (int j = 0; j < i; j++)
      msg_i[j] = i;
    auto mac = Poly1305(Span<>(msg_i, i), key_i);
    total_ctx.Update(mac.bytes);
  }

  Poly1305 total = total_ctx.Finalize();
  EXPECT_EQ(BytesToHex(total.bytes), "64afe2e8d6ad7bbdd287f97c44623d39");
}