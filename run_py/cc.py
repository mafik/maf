...
# TODO: add either CXXFLAGS_RELEASE or CXXFLAGS_DEBUG depending on the build type
...
def AddSteps(recipe):
    ...
    for path, deps in graph.items():
        redir_graph[redirect_path(path)] = [redirect_path(d) for d in deps]

    for path, t in types.items():
        redir_types[redirect_path(path)] = t
    ...
    for path, deps in redir_graph.items():
        ...
        if t in ('header', 'translation unit'):
            ...
            source_files = [d for d in deps if redir_types[d]
                            == 'translation unit']
            assert len(
    ...
    tests = [p for p, t in redir_types.items() if t == 'test']
    if tests:
        # run all tests sequentially
        def run_tests(extra_args):
            for test in tests:
                run([f'./{test}'] + TEST_ARGS + extra_args, check=True)
