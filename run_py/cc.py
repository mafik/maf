...
def AddSteps(recipe):
    ...
    # This section duplicates some of the graph elements to account for build variants & redirects their paths.
    ...
    def redirect_path(path):
        ...
        name = path.name
        path = str(path)
        if types[path] == 'object file':
            return str(OBJ_DIR / name)
        elif types[path] == 'test':
            return str(TEST_DIR / name)
        elif types[path] == 'main':
    ...
    def debug_prefix(path_str):
        path = Path(path_str)
        return str(path.with_stem('debug_' + path.stem))

    def release_prefix(path_str):
        path = Path(path_str)
        return str(path.with_stem('release_' + path.stem))

    redir_types = dict()
    redir_graph = dict()
    variant = collections.defaultdict(str)
    for path, deps in graph.items():
        default_path = redirect_path(path)
        debug_path = debug_prefix(default_path)
        release_path = release_prefix(default_path)
        if types[path] == 'object file':
            # object files have three variants with the same deps (input source files)
            new_deps = [redirect_path(d) for d in deps]
            redir_graph[default_path] = new_deps
            redir_types[default_path] = 'object file'
            variant[default_path] = 'default'
            redir_graph[release_path] = new_deps
            redir_types[release_path] = 'object file'
            variant[release_path] = 'release'
            redir_graph[debug_path] = new_deps
            redir_types[debug_path] = 'object file'
            variant[debug_path] = 'debug'
        elif types[path] in ('test', 'main'):
            redir_graph[default_path] = [redirect_path(d) for d in deps]
            redir_types[default_path] = types[path]
            variant[default_path] = 'default'
            redir_graph[release_path] = [
                release_prefix(redirect_path(d)) for d in deps]
            redir_types[release_path] = types[path]
            variant[release_path] = 'release'
            redir_graph[debug_path] = [
                debug_prefix(redirect_path(d)) for d in deps]
            redir_types[debug_path] = types[path]
            variant[debug_path] = 'debug'
        else:
            redir_graph[default_path] = [redirect_path(d) for d in deps]
            redir_types[default_path] = types[path]
    ...
    # Shortcut recipe for running all tests (default build variant)
    tests = [p for p, t in redir_types.items(
    ) if t == 'test' and variant[p] == 'default']
    if tests:
